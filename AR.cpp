...
private:
  ...
  Object AR_Init(const Tuple &a, const Dict &kws) {
    ...
    if (kws.hasKey("threshold")) {
      ...
      cout << "Setting threshold from init: " << (int)val << endl;
  ...
  Object AR_Run(const Tuple &a) {
    // setup camera/window information
    setup.SetupWindow();
    ...
    // setup OpenCV interface
    arcv.Init(setup.GetWinWidth(),setup.GetWinHeight());

    // Call python-specified init function if it exists    
    if (Library.hasKey("initFunc")) {
      if (Library["initFunc"].isCallable()) {
        std::cout << "Calling python init func" << endl;
  ...
  static void takeCvNormalScreenCap() {
    ...
    cout << normal << endl;
    cout << cvSaveImage("cvScreenCapNormal.jpg",normal) << endl;
  }
  static void takeCvGrayScreenCap() {
    IplImage *gray = arcv.GetGrayscale();
    cout << gray << endl;
    cout << cvSaveImage("cvScreenCapGray.jpg",gray) << endl;
  }
  ...
  static void MainLoop() {
    ...
    // Save OpenCV image information
    arcv.SaveCameraCapture(imageData);
