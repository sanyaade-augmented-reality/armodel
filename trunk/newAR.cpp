...
class newARLogger : public ARToolKitPlus::Logger
{
...
public:
  ...
  virtual ~newAR_module()
  {
    std::cout << "Cleaned up AR module" << std::endl;
  }

private:
  Object newAR_GetImage(const Tuple &a, const Dict &kws) {
    return (Int)((int)frame);
  }
  
  Object newAR_Init(const Tuple &a, const Dict &kws) {
    //////////////////////////////////////////////////////////////
  ...
  Object newAR_Run(const Tuple &a) {
  ...
  Object newAR_GetFound(const Tuple &a) {
    /*
      typedef struct {
      char   name[256];
      int    id;
  ...
  Object newAR_GetMulti(const Tuple &a) {
    int i,j,k;
    if (a.size() > 0) 
      throw Exception("No arguments");
  ...
  Object newAR_SetMultiLoop(const Tuple &a) {
    if (! (a[0].isCallable()) )
  ...
  Object newAR_SetSingleLoop(const Tuple &a) {
    if (! (a[0].isCallable()) )
      throw Exception("Single argument must be a function");
  ...
  Object newAR_SetMultiMV(const Tuple &a) {
    /*
      argDrawMode3D();
      argDraw3dCamera( 0, 0 );
      glMatrixMode(GL_MODELVIEW);
...
extern "C" void initnewAR()
{
#if defined(PY_WIN32_DELAYLOAD_PYTHON_DLL)
  InitialisePythonIndirectPy::Interface();
#endif

  static newAR_module* newAR = new newAR_module;
}

// symbol required for the debug version
extern "C" void initnewAR_d()
{ initnewAR(); }
